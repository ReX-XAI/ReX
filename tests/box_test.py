...
data_3d = Data(input=np.random.rand(1, 64, 64, 64), model_shape=[1, 64, 64, 64], device="cpu")
...
def test_initialise_tree():
    box_3d = initialise_tree(r_lim=64, c_lim=64, d_lim=64,
                    r_start=0, c_start=0, d_start=0, distribution="uniform", distribution_args=None)
    box_2d = initialise_tree(r_lim=64, c_lim=64, r_start=0,
                             c_start=0, distribution="uniform", distribution_args=None)
    ...
    assert box_3d.__repr__() == "Box < name: R, row_start: 0, row_stop: 64, col_start: 0, col_stop: 64, depth_start: 0, depth_stop: 64, area: 262144"
    assert box_2d.__repr__() == "Box < name: R, row_start: 0, row_stop: 64, col_start: 0, col_stop: 64, area: 4096"
    ...
    assert box_2d.corners() == (0, 64, 0, 64) #TODO: Box_dimensions has the same functionality as corners, should we remove one of them.

def test_spawn_children():
    box_3d = initialise_tree(r_lim=64, c_lim=64, d_lim=64,
                    r_start=0, c_start=0, d_start=0, distribution=Distribution.Uniform, distribution_args=None)
    box_2d = initialise_tree(r_lim=64, c_lim=64, r_start=0,
                             c_start=0, distribution=Distribution.Uniform, distribution_args=None)
    ...
    total_area_3d = children_3d[0].area() +  children_3d[1].area() + children_3d[2].area() + children_3d[3].area()
    assert total_area_3d == 262144

    total_area_2d = children_2d[0].area() +  children_2d[1].area() + children_2d[2].area() + children_2d[3].area()
